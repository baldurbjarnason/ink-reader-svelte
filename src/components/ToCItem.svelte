<script>
  // Need to figure something about setting current page 
  import {url, urlPrefix} from "../stores"
  export let item
  let selected
  $: if ($url && $url.join("/") === item.url) {
    selected = true
  } else {
    selected = false
  }
...
<li class:selected class="ContentsItem">
  ...
  {:else}
   {#if item.url}
    <a href="{$urlPrefix}/{item.url}" class="Level">{item.label}</a>
   {:else}
